    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (args.length == 0) {
            if (!(sender instanceof Player player)) {
                sender.sendMessage("[CivEvents] Console can't use this command - /gms");
                return true;
            }
            ...
            if (target != null && target.isOnline()) {
                ...
                if (!(sender instanceof Player player)) {
                    sender.sendMessage("[CivEvents] " + target.getName() + " is now in survival mode");
                    return true;
                } else {
                    sender.sendMessage("§f§lCivEvents §f| §a" + target.getName() + " is now in survival mode");
                    return true;
                }
            } else {
                if (!(sender instanceof Player player)) {
                    sender.sendMessage("[CivEvents] Player not found - /gms" + target.getName());
                    return true;
                } else {
